 ...
 *
 */
export class MapWall extends RotMap {

    public map: number[][];
    public _options: {[key: string]: any};

    protected _wallNS: any[][];
    protected _wallEW: any[][];
...
/* Gets the width using moving average algorithm. */
    getWidthMovingAvg(
        ...
        for (let i = filterW; i < (nElem - filterW); i++) {
            const filtVal = windowFilter(unfiltered, i, filterW);
    ...
    getWallWidth(mean: number, stddev: number, subSize: number): number {
...
function windowFilter(arr: number[], i: number, filterW: number): number {
