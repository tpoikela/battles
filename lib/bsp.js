...
const Room = function( container ) {
    ...
    if (this.w < OPTS.MIN_ROOM_W) {this.w = OPTS.MIN_ROOM_W;}
    if (this.h < OPTS.MIN_ROOM_H) {this.h = OPTS.MIN_ROOM_H;}
    return this;
};
...
const BSPGen = function(opts = OPTS) {
    this._opts = opts;
};
BSP.BSPGen = BSPGen;
...
BSPGen.prototype.isLargeEnough = function(cont) {
    if (this._opts.DISCARD_BY_SIZE) {
        return cont.w >= this._opts.MIN_SPLIT_W && cont.h >= this._opts.MIN_SPLIT_H;
    }
    return true;
};
...
BSPGen.prototype.randomSplit = function(container) {
    let r1, r2;
    if (rng.getUniform() <= this._opts.VERT_SPLIT) {
        // Vertical
        r1 = new Container(
        ...
        if (this._opts.DISCARD_BY_RATIO) {
...
BSPGen.prototype.isVRatioTooSmall = function(r1, r2) {
    ...
    return r1Wratio < this._opts.W_RATIO || r2Wratio < this._opts.W_RATIO;
};

BSPGen.prototype.isHRatioTooSmall = function(r1, r2) {
    const r1Hratio = r1.h / r1.w;
    const r2Hratio = r2.h / r2.w;
    return r1Hratio < this._opts.H_RATIO || r2Hratio < this._opts.H_RATIO;
};
