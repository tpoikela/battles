...
Territory.prototype.setAsEmpty = function(map, cellInfo) {
    ...
    for (let x = 0; x < map.length; x++) {
        for (let y = 0; y < map[0].length; y++) {
            if (cellInfo[map[x][y]]) {
                console.log('Marking empty', x, y, 'with', map[x][y]);
                this._markEmpty(x, y);
            }
            else {
                this.map[x][y] = FULL;
...
Territory.prototype.generate = function() {
    ...
    while (this.hasEmpty() && this.contestants.length > 0) {
        ...
        const {open, isFirst} = contData;

        // If no cells occupied, pick one randomly
        if (isFirst) {
            const xy = this.getEmptyXY();
            if (contData.startX >= 0) {
                xy[0] = contData.startX;
            }
            if (contData.startY >= 0) {
                xy[1] = contData.startY;
            }
            this.addOccupied(name, xy);
            contData.isFirst = false;
            console.log(name, ' 1st cell is', xy);
            this.contestants.push(next);
        }
        else if (Object.keys(open).length > 0) {
            console.log(name, 'looking for an open cell');
...
Territory.prototype.addOccupied = function(name, xy) {
    ...
    // 1st cell can be forced to be non-empty
    if (this.empty[key]) {
        --this.numEmpty;
...
Territory.prototype.getEmptyXY = function() {
    return RNG.arrayGetRand(Object.values(this.empty));
};

Territory.prototype.getOpenXY = function(name) {
    const {open} = this.data[name];
    return RNG.arrayGetRand(Object.values(open));
};
