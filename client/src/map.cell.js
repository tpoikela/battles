var RG = require('./rg');
var TYPE_ACTOR = RG.TYPE_ACTOR, TYPE_ITEM = RG.TYPE_ITEM, TYPE_ELEM = RG.TYPE_ELEM;
...
Cell.prototype.getFirstActor = function () {
    var actors = this.getProp(TYPE_ACTOR);
...
Cell.prototype.getSentientActors = function () {
    ...
    return actors.filter(function (actor) { return !actor.has('NonSentient'); });
};
...
Cell.prototype.hasStairs = function () {
    var propType = this.getConnection();
    if (propType) {
        var name_1 = propType.getName();
        return (/stairs(Up|Down)/).test(name_1);
    }
    return false;
};
...
Cell.prototype.hasPassage = function () {
    var propType = this.getConnection();
    if (propType) {
        return propType.getName() === 'passage';
    }
    return false;
};
...
Cell.prototype.lightPasses = function () {
    ...
    if (!this._lightPasses) {
        return false;
    }
    // if (this.hasProp(TYPE_ELEM)) {
    var elems = this._p[TYPE_ELEM];
    if (elems) {
        if (elems.length === 1) {
            if (elems[0].has('Opaque')) {
                return false;
            }
        }
        else {
...
Cell.prototype.isFree = function (isFlying) {
    if (isFlying === void 0) { isFlying = false; }
    // if (!isFlying && !this._baseElem.isPassable()) {return false;}
    if (!isFlying && !this._isPassable) {
        return false;
    }
...
Cell.prototype.setProp = function (prop, obj) {
    if (obj.getType() === 'connection' && this.hasConnection()) {
        var msg = this._x + "," + this._y;
        msg += "\nExisting: " + JSON.stringify(this.getConnection());
        msg += "\nTried to add: " + JSON.stringify(obj);
        RG.err('Cell', 'setProp', "Tried to add 2nd connection: " + msg);
    }
    if (!this._p.hasOwnProperty(prop)) {
        this._p[prop] = [];
...
Cell.prototype.hasUsable = function () {
    var elems = this.getProp(RG.TYPE_ELEM);
    if (elems) {
        for (var i = 0; i < elems.length; i++) {
            if (elems[i].onUse) {
                return true;
    ...
    if (this._p.hasOwnProperty(RG.TYPE_ELEM)) {
        ...
        this._p[RG.TYPE_ELEM].forEach(function (elem) {
            if (/(snow|tree|grass|stone|water)/.test(elem.getType())) {
                elements_1.push(elem.toJSON());
            }
        });
        if (elements_1.length > 0) {
            json.elements = elements_1;
        }
    }
    return json;
};
...
Cell.prototype.getPropNames = function () {
    var _this = this;
    var result = [this._baseElem.getType()];
    var keys = Object.keys(this._p);
    keys.forEach(function (propType) {
        var props = _this.getProp(propType);
...
Cell.prototype.findObj = function (filterFunc) {
    var _this = this;
    var result = [];
    Object.keys(this._p).forEach(function (propType) {
        var props = _this._p[propType];
